...
- **Build/test/dev commands.** `make dev`, `make run`, `make build` (fast local/dev), `make build-prod` (optimized host production), `make build-linux-amd64` (optimized static Linux AMD64 production), `make build-darwin-arm64`, `make build-all-platforms`, `make test`, `make check`, `make assets`, `make migrate-up` / `-down` / `-status`, `make install-hooks`.
